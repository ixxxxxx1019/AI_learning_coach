...
>
> **v0.2.0** — 生产级工程化改造：用户登入系统、学习记忆持久化、结构化日志、LLM 韧性（重试/熔断/缓存）、Prompt 外部化、Docker 容器化、CI/CD、单元测试、FastAPI REST API。
...
| **用户认证** | bcrypt + SHA256 Token | 注册/登入，Token 验证，学习进度按用户隔离 |
| **学习记忆** | ProgressStore (JSON) | 知识点掌握度、SM-2 状态、学习历史跨 session 持久化 |
...
│   └── auth.py                  # [NEW] 用户认证：注册/登入/bcrypt/Token
│
...
│  Swagger UI /docs   │  │  ⓪ Login ① Setup ② Learn ③ Result│
│  10 REST Endpoints  │  │  用户认证 + 学习记忆持久化       │
└──────────┬──────────┘  └────────────────┬────────────────┘
           │                              │
           └──────────┬───────────────────┘
...
| `POST` | `/api/auth/login` | 登入 → 返回 Bearer token |
| `GET` | `/api/auth/me` | 当前用户信息 + 学习统计（需 token） |
| `GET` | `/api/health` | 健康检查（配置 / KG / API 三重校验） |
| `GET` | `/api/subjects` | 列出所有学科 |
| `GET` | `/api/subjects/{id}/kps` | 列出某学科知识点（含依赖/难度/标签） |
| `POST` | `/api/sessions` | 创建学习 session（需 token，自动加载历史进度） |
| `GET` | `/api/sessions/{id}` | 查询 session 当前状态（需 token） |
| `POST` | `/api/sessions/{id}/answers` | 提交答案 → grade + diagnosis（需 token） |
| `DELETE` | `/api/sessions/{id}` | 删除 session（需 token） |
...
- **用户登入系统**：bcrypt 密码哈希 + SHA256 Token + 注册/登入/验证
- **学习记忆持久化**：Planner 自动加载已掌握 KP、优先薄弱环节、历史对比
- **FastAPI REST API**：10 个端点（含 3 个认证端点）+ Token 保护 + Swagger 文档
- **用户进度**：`ProgressStore` — SM-2 状态按用户隔离持久化
